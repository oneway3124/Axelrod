...
class TestContriteTitForTat(TestPlayer):
    ...
    def test_strategy_with_noise(self):
        ...
        match = axelrod.Match((player, opponent), turns=1, seed=9)
        match.play()
        self.assertEqual(player.history[-1], C)
        self.assertEqual(player._recorded_history, [C])

        match = axelrod.Match((player, opponent), turns=1, noise=0.9, seed=9)
        match.play()
...
class TestNTitsForMTats(TestPlayer):
    ...
    def test_strategy(self):
        ...
        self.versus_test(
            axelrod.Random(), expected_actions=actions, seed=2, init_kwargs=init_kwargs
        )
        opponent = axelrod.MockPlayer(actions=[C, D])
        actions = [(C, C), (C, D), (D, C), (C, D)]
